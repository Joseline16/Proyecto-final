...
from bd_conexion import mostrar_datos,eliminar_datos
...
class VentanaConsulta(tk.Tk):
    ...
    # Parte de los botones (frame_buscar)
        ...
        self.btn_buscar = tk.Button(self.frame_buscar,text="Buscar", width=10,height=1,command=self.buscar).grid(row=0,column=2 ,padx=15,pady=20)

    #parte tablas (frame_tabla)

        self.scrol_y = ttk.Scrollbar(self.frame_tabla,orient=tk.VERTICAL)
        self.scrol_y.pack(side=tk.RIGHT,fill=tk.Y)

        self.scrol_x = ttk.Scrollbar(self.frame_tabla,orient=tk.HORIZONTAL)
        self.scrol_x.pack(side=tk.BOTTOM,fill=tk.X)

        self.tabla = ttk.Treeview(self.frame_tabla,columns=("ID","Nombre","Cantidad","Categoría","proveedor"),show="headings",height=15, yscrollcommand=self.scrol_y.set, xscrollcommand=self.scrol_x.set)
    ...
    def cargar_datos(self, filtro=None):
        ...
        productos = mostrar_datos(filtro)
